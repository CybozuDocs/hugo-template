...
describe('processImages', () => {
  describe('基本的な画像変換', () => {
    it('シンプルな画像をImgコンポーネントに変換する', () => {
      const input = '![スクリーンショット：ログイン画面](/k/img-ja/to_be_invited_03.png)';
      const pathReplacement: PathReplacement = { from: '/k/', to: '/k/kintone/' };
      const result = processImages(input, pathReplacement);
      
      expect(result.content).toBe('<Img src="/k/kintone/img-ja/to_be_invited_03.png" alt="スクリーンショット：ログイン画面" />');
      expect(result.imports).toContain('import Img from "@/components/Img.astro";');
    ...
    it('タイトル属性付き画像を変換する', () => {
      ...
      expect(result.content).toBe('<Img src="/k/kintone/img-ja/sample.png" alt="代替テキスト" title="タイトル" />');
      expect(result.converted).toBe(true);
    });
    ...
    it('空のalt属性を持つ画像を変換する', () => {
      ...
      expect(result.content).toBe('<Img src="/k/kintone/img-ja/sample.png" alt="" />');
      expect(result.converted).toBe(true);
    });
  });
  ...
  describe('パス変換', () => {
    it('指定されたパスプレフィックスを変換する', () => {
      ...
      expect(result.content).toBe('<Img src="/k/kintone/img-ja/sample.png" alt="画像" />');
    });
    ...
    it('パスプレフィックスが一致しない場合は変換しない', () => {
      ...
      expect(result.content).toBe('<Img src="/other/img-ja/sample.png" alt="画像" />');
    });
    ...
    it('パス変換なしでも正常に動作する', () => {
      ...
      expect(result.content).toBe('<Img src="/k/img-ja/sample.png" alt="画像" />');
    });
  });
...
段落3の内容`;
      ...
      expect(result.imports[0]).toBe('import Img from "@/components/Img.astro";');
    });
  });

  describe('エッジケース', () => {
    it('画像がない場合は内容を変更しない', () => {
      const input = '通常のテキストです。[リンク](https://example.com)もあります。';
      const result = processImages(input);
    ...
    it('画像内に特殊なパターンがある場合でも正しく処理する', () => {
      ...
      expect(result.content).toBe('<Img src="/k/kintone/img-ja/sample.png" alt="画像 [内部] (テキスト)" />');
    });
