...
function fixHtmlTagFormatting(content: string): string {
  ...
  const tagsToFormat = [
    ...
    "blockquote", // Quotes
    "figure",
    "figcaption", // Figure elements
    "header",
    "footer", // Semantic sections
    "nav",
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match && !line.includes(`</${tagName}>`)) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        const openTags = (
          checkLine.match(new RegExp(`<${tagName}[^>]*>`, "g")) || []
        ).length;
        const closeTags = (
          checkLine.match(new RegExp(`</${tagName}>`, "g")) || []
        ).length;
      ...
      if (closingTagLine !== -1) {
        ...
        if (closingTagIndex > 0) {
          const beforeClosing = closingLine
            .substring(0, closingTagIndex)
            .trim();
          if (beforeClosing) {
    ...
    const unbalancedOpenRegex = new RegExp(
