...
export function processShortcodes(content: string): ConversionResult {
  ...
  processedContent = processContentShortcodes(
    processedContent,
    imports,
    errors,
  );
  ...
  processedContent = processAttributeShortcodes(
    processedContent,
    imports,
    errors,
  );
  ...
  for (const [shortcode, component] of Object.entries(
    ...
    if (shortcode === "wv_brk") {
      ...
      for (const match of blockMatches) {
        ...
        const hasProblematicIndentation = checkForProblematicIndentation(
          innerContent,
          indent,
        );
...
function processAttributeShortcodes(
  ...
  for (const [shortcode, allowedAttrs] of Object.entries(
    SHORTCODE_MAPPINGS.attributes,
  )) {
...
function checkForProblematicIndentation(
  content: string,
  baseIndent: string,
): boolean {
