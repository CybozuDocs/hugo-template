...
describe('processHeadings', () => {
  describe('カスタムID付き見出しの変換', () => {
    it('レベル2の見出しをHeadingコンポーネントに変換する（level属性なし）', () => {
      ...
      expect(result.content).toBe('<Heading id="start_whatskintone_10">アプリ</Heading>');
      expect(result.imports).toContain('import Heading from "@/components/Heading.astro";');
    ...
    it('レベル1の見出しをlevel属性付きで変換する', () => {
      ...
      expect(result.content).toBe('<Heading level={1} id="custom-id">タイトル</Heading>');
      expect(result.converted).toBe(true);
    });
    ...
    it('レベル3の見出しをlevel属性付きで変換する', () => {
      ...
      expect(result.content).toBe('<Heading level={3} id="sub-section">サブセクション</Heading>');
      expect(result.converted).toBe(true);
    });
    ...
    it('レベル4の見出しをlevel属性付きで変換する', () => {
      ...
      expect(result.content).toBe('<Heading level={4} id="detail">詳細</Heading>');
      expect(result.converted).toBe(true);
    });
    ...
    it('レベル5の見出しをlevel属性付きで変換する', () => {
      ...
      expect(result.content).toBe('<Heading level={5} id="small">小見出し</Heading>');
      expect(result.converted).toBe(true);
    });
    ...
    it('レベル6の見出しをlevel属性付きで変換する', () => {
      ...
      expect(result.content).toBe('<Heading level={6} id="tiny">最小見出し</Heading>');
      expect(result.converted).toBe(true);
    });
  });
...
## 別のサブ`;
  ...
  describe('エッジケース', () => {
    it('見出し内に中括弧がある場合も正しく処理する', () => {
      ...
      expect(result.content).toBe('<Heading id="object">オブジェクト{key: value}の説明</Heading>');
      expect(result.converted).toBe(true);
    });
    ...
    it('見出しの前後の空白を保持する', () => {
      ...
      expect(result.content).toBe('<Heading id="spaced">スペース付き見出し</Heading>');
      expect(result.converted).toBe(true);
    });
