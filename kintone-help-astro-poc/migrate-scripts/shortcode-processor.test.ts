...
[ログインに関するトラブル](/general/ja/id/020262.html)
  ...
  describe("Attributes型ショートコード", () => {
    it("{{< subnavi >}} with attributes をSubnaviコンポーネントに変換する", () => {
      const input = `{{<subnavi
msg="Please contact us if you cannot find the information you are looking for."
url="/k/en/id/040321.html"
inquiry="Contact Us"
>}}`;
      const result = processShortcodes(input);

      const expected = `<Subnavi msg="Please contact us if you cannot find the information you are looking for." url="/k/en/id/040321.html" inquiry="Contact Us" />`;
      expect(result.content).toBe(expected);
      expect(result.imports).toContain(
    ...
    it("{{< subnavi >}} with Japanese attributes をSubnaviコンポーネントに変換する", () => {
      const input = `{{<subnavi
msg="ヘルプで見つからない情報は、サポートにお問い合わせください。"
url="/k/ja/id/040321.html"
inquiry="お問い合わせ"
>}}`;
      const result = processShortcodes(input);

      const expected = `<Subnavi msg="ヘルプで見つからない情報は、サポートにお問い合わせください。" url="/k/ja/id/040321.html" inquiry="お問い合わせ" />`;
      expect(result.content).toBe(expected);
      expect(result.imports).toContain(
        'import Subnavi from "@/components/Subnavi.astro";',
      );
    ...
    it("enabled2内にsubnaviが含まれるパターンを処理する", () => {
...
日本限定のコンテンツ
    ...
    it("インライン形式のenabled2を正しく変換する", () => {
      const input =
        "グループウェア{{< enabled2 JP >}}（Garoonなど）{{< /enabled2 >}}のようなスケジュール機能はありません。";
      const result = processShortcodes(input);
    ...
    it("enabled2が誤ってEnabledに属性形式で変換されないことを確認", () => {
      ...
      // 期待しない結果のパターン
      const unexpectedPattern1 =
        'グループウェア<Enabled2 regions={["JP"]} />（Garoonなど）';
...
<Kintone />の使い方について説明します。
  ...
  describe("エッジケース", () => {
    ...
    it("重複するコンポーネントのインポートは1つにする", () => {
      ...
      const kintoneImports = result.imports.filter((imp) =>
        imp.includes("Kintone"),
      );
  ...
  describe("ネストした複雑なケース", () => {
    it("wv_brk内にテキストが含まれるパターンを処理する", () => {
      const input =
        "{{< wv_brk >}}［レコードの一覧］{{< /wv_brk >}}画面で、レコードの編集や削除ができます。";
      const result = processShortcodes(input);
...
* テーブル内のフィールドでは、使用できない機能や操作があります（例：ルックアップ、関連レコード一覧、フィールドのアクセス権など）。
    ...
    it("複数のwv_brkが連続するパターンを処理する", () => {
      const input =
        "アプリはポータルのお知らせ掲示板や、スペースのお知らせ（スペースの本文）、スレッドの本文に貼り付けることができます。{{< wv_brk >}}{{< /wv_brk >}}{{< wv_brk >}}{{< /wv_brk >}}  \n貼り付けられたアプリのレコード一覧からも、{{< wv_brk >}}［レコードの一覧］{{< /wv_brk >}}画面と同様の手順で、レコードの編集や削除ができます。";
      const result = processShortcodes(input);
