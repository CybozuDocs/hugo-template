...
async function main(): Promise<void> {
  try {
    ...
    if (config.imagePathPrefix) {
      console.log(
        `🖼️  Image path: ${config.imagePathPrefix.from} → ${config.imagePathPrefix.to}`,
      );
    }
...
async function convertFiles(
  ...
  if (config.preprocessorConfig) {
    try {
      preprocessorConfig = await loadPreprocessorConfig(
        config.preprocessorConfig,
      );
      console.log(
        `🔧 Loaded ${preprocessorConfig.rules.length} preprocessor rules`,
      );
    } catch (error) {
  ...
  for (const inputFile of files) {
    try {
      ...
      if (stats.processedFiles % 10 === 0) {
        console.log(
          `📊 Progress: ${stats.processedFiles}/${files.length} files processed`,
        );
      }
    } catch (error) {
...
async function convertFile(
  ...
  const preprocessingResult = applyPreprocessing(
    inputFile,
    bodyContent,
    preprocessorConfig,
  );
  ...
  if (preprocessingResult.rulesApplied.length > 0) {
    console.log(
      `   🔧 Preprocessor applied: ${preprocessingResult.rulesApplied.join(", ")}`,
    );
  }
...
function printStats(stats: ProcessingStats): void {
  ...
  if (stats.processedFiles > 0) {
    const conversionRate = Math.round(
      (stats.convertedFiles / stats.processedFiles) * 100,
    );
...
export function analyzeFile(filePath: string): Promise<void> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(
        "🖼️  Images found:",
        processImages(bodyContent).imports.length > 0,
      );
      console.log(
        "🔧 Shortcodes found:",
        processShortcodes(bodyContent).imports.length > 0,
      );
