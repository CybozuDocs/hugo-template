...
describe('processEscaping', () => {
  describe('JSON様構造のエスケープ', () => {
    ...
    it('複数のキーを持つオブジェクトをエスケープする', () => {
      ...
      expect(result.content).toBe('オプション: &#123;name: "test", value: 123, enabled: true&#125;');
      expect(result.converted).toBe(true);
    });
    ...
    it('ネストされたオブジェクトをエスケープする', () => {
      ...
      expect(result.content).toBe('データ: &#123;user: &#123;name: "John", age: 30&#125;&#125;');
      expect(result.converted).toBe(true);
    });
    ...
    it('配列を含むオブジェクトをエスケープする', () => {
      ...
      expect(result.content).toBe('設定: &#123;items: [1, 2, 3], enabled: true&#125;');
      expect(result.converted).toBe(true);
    });
  ...
  describe('JSXコンポーネントの除外', () => {
    ...
    it('属性付きコンポーネントもエスケープしない', () => {
      const input = '<Img src="/path/to/image.png" alt="画像" />と<Heading level={2}>見出し</Heading>';
      const result = processEscaping(input);
...
インライン: \`&#123;key: value&#125;\``;
  ...
  describe('エッジケース', () => {
    ...
    it('複数のオブジェクトを一度にエスケープする', () => {
      ...
      const expected = '設定1: &#123;a: 1&#125; と設定2: &#123;b: 2&#125; と設定3: &#123;c: 3&#125;';
      expect(result.content).toBe(expected);
      expect(result.converted).toBe(true);
    });
