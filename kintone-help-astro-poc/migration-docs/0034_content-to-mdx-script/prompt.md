...
- 属性クォート修正
- テキスト行とタグ分離
- インデント不一致修正（MDXコンポーネントの開始・終了タグ）
- wv_brkショートコード処理の分離（インライン vs ブロック）
- Noteコンテンツのインデント統一（プリプロセッサによる個別修正）
- プリプロセッサ動作確認手順の確立（ダミー変換→実処理）
- Note内コンテンツインデント自動調整機能（shortcode-processor.ts）
  - 閉じタグインデント修正、改行重複防止処理も実装
